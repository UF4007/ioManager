...
io::fsm_func<void> stackful_benchmark();
...
io::fsm_func<void> stackful_benchmark()
{
    ...
    for (size_t i = 0; i < NUM_COROS; i++)
    {
        io::stackful::spawn_stacksize(1024, [i](std::vector<io::promise<void>> &promises)
                                      {
                size_t j = i;
                io::future fut;
                while (1)
                {
                    io::promise<void> prom = io::make_future(fut);
                    promises[j] = std::move(prom);
                    io::stackful::await(fut);
                    if (fut.getErr())
                        return;
                } }, test_promises);
    }
    ...
    for (size_t k = 0; k < ITERATION - 1; k++)
    {
        for (size_t i = 0; i < NUM_COROS; i++)
        {
            test_promises[i].resolve();
        }
    }
    for (size_t i = 0; i < NUM_COROS; i++)
    {
        test_promises[i].reject(std::errc::operation_canceled);
    }

    auto duration_loop = std::chrono::duration_cast<std::chrono::microseconds>(timer.lap());
    double creates_per_sec = NUM_COROS * 1000000.0 / duration_coro.count();
    double switches_per_sec = NUM_COROS * ITERATION * 1000000.0 / duration_loop.count();
    ...
    std::cout << "Stackful Benchmark Results:\n"
              ...
              << "Spawn per second: " << static_cast<size_t>(creates_per_sec) << "\n"
              << "Average spawn time: " << duration_coro.count() / double(NUM_COROS) * 1000.0 << " ns\n\n"
              << "Total switches: " << NUM_COROS * ITERATION << "\n"
              << "Total time: " << duration_loop.count() / 1000.0 << " ms\n"
              << "Switches per second: " << static_cast<size_t>(switches_per_sec) << "\n"
              << "Average switch time: " << duration_loop.count() / double(NUM_COROS * ITERATION) * 1000.0 << " ns\n\n\n\n";
