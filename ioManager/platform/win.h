...
//netif
struct io::netif::windows_netif_handle {
	inline windows_netif_handle() {
		DWORD dwSize = 0;
		DWORD dwRetVal = 0;
		ULONG flags = GAA_FLAG_INCLUDE_PREFIX;
		dwRetVal = GetAdaptersAddresses(AF_UNSPEC, flags, NULL, handle, &dwSize);
		if (dwRetVal == ERROR_BUFFER_OVERFLOW) {
			handle = (PIP_ADAPTER_ADDRESSES)::operator new(dwSize);
		}
	}
	inline ~windows_netif_handle() {
		if (handle)
	...
	PIP_ADAPTER_ADDRESSES handle = nullptr;
};
inline void io::netif::global_startup() {
	if (_is_init_netif.test_and_set() == false)
	{
		...
		if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
			throw std::runtime_error("windows netif initialize error!");
		}
...
inline io::err io::netif::reload() {
	_handle = std::make_shared<io::netif::windows_netif_handle>();
...
inline io::err io::udp_socket::sendto(const char* c, size_t s, const sockaddr_in& addr) {
	if (handle == INVALID_SOCKET)
		return io::err::failed;
	int result = ::sendto(handle, c, s, 0, (sockaddr*)&addr, sizeof(addr));
	if (result == SOCKET_ERROR)
		return io::err::failed;
	return io::err::ok;
}
